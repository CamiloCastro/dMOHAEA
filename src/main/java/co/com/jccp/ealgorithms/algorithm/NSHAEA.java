...
public class NSHAEA<T> {
    ...
    public List<MOEAIndividual<T>> apply()
    {
        ...
        for (MOEAIndividual<T> individual : pop) {
            double[] op = new double[operators.size()];
            for (int i = 0; i < operators.size(); i++) {
                op[i] = 1.0 / (double) operators.size();
            }
            individual.setOpProbabilities(op);
        }
        ...
        for (int t = 0; t < iterations; t++) {
            ...
            if(initAgain)
            {
                pop = popInit.init(popSize, dimensions);
                calculateObjectives(pop);
                FNDS.apply(pop, minimize);
                for (MOEAIndividual<T> individual : pop) {
                    double[] op = new double[operators.size()];
                ...
                offspring = generateOffspring(pop, MOEAIndividual::compareByRank);
                t = 0;
                continue;
            }
